...
- **当前阶段**：**Phase 2 — 发现与连接层** ✅ 完成；下一步 **Phase 3**（更新/下载安装协调：UpdateCoordinator + DownloadInstallController + LocalAppRepository）
- **阶段状态**：✅ Phase 0/1/2 完成（Phase 2 待用户审阅）
- **测试基线**：**117/117 全绿**（Phase 2 末，`testDebugUnitTest` + `assembleDebug` 均通过，见 §4）。
...
| **Phase 3** | 更新/下载安装协调（UpdateCoordinator + DownloadInstallController + LocalAppRepository） | 阶段 4 其余 | ⏳ 待启动 | — |
| **Phase 4** | 接线与门面切换（整合进 AppRepository / 瘦门面 / 删除旧码 / DI） | 阶段 3+4 | ⏳ 待启动 | 消除 legacy-known-issue |
